...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen container mx-auto pt-6">
      {/* Full-width header */}
      <header className="border-border border px-6 max-w-6xl mx-auto bg-white/50 rounded-full">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h1 className="text-xl p font-bold tracking-tight text-cyan-950">Daily Open Video Relevant Discussion Tracker</h1>
              <p className="text-xs font-medium text-cyan-800 mt-1">Tylerbot finds discussions. New each day. Most suck, some are winners.</p>
            </div>
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                disabled={loading}
                className="inline-flex text-sm items-center gap-2 px-3 py-2 bg-cyan-900 text-cyan-100 rounded-full hover:bg-cyan-700/50 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} />
                {loading ? 'Fetching...' : 'Refresh'}
              </button>
              {lastFetch && (
                <div className="text-xs text-cyan-900 font-mono flex items-center gap-1 px-2 py-0.5 ">
